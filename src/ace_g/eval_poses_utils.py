...
"""Utility functions for evaluating poses."""
import logging
import math
import random
...
# Poses are represented as 4x4 transformation matrices and are cam-to-world transformations.
TestEstimate = namedtuple("TestEstimate", ["pose_c2w_est", "pose_c2w_gt", "calibration", "confidence", "image_file"])
...
def print_hyp(hypothesis, hyp_name):
    ...
    _logger.debug(
        f"{hyp_name}: score={hypothesis['score']}, translation={h_translation:.2f}m, " f"rotation={h_angle:.1f}deg."
    )
...
def estimate_alignment(
    ...
    valid_estimates = [
        ...
        if ((not np.any(np.isinf(estimate.pose_c2w_gt))) and (not np.any(np.isnan(estimate.pose_c2w_gt))))
    ]
    ...
    for i, estimate in enumerate(confident_estimates):
        poses_est[i] = estimate.pose_c2w_est
        poses_gt[i] = estimate.pose_c2w_gt
