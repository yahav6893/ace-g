...
def get_base_file_name(file_name):
    ...
    E.g. frame-000000.color.jpg -> frame-000000
    """
...
def write_cam_pose(file_path, cam_pose):
    ...
    with open(file_path, "w") as f:
        f.write( 
            str(float(cam_pose[0, 0])) + " " + str(float(cam_pose[0, 1])) + " " + 
            str(float(cam_pose[0, 2])) + " " + str(float(cam_pose[0, 3])) + "\n"
        )
        f.write(
            str(float(cam_pose[1, 0])) + " " + str(float(cam_pose[1, 1])) + " " + 
            str(float(cam_pose[1, 2])) + " " + str(float(cam_pose[1, 3])) + "\n"
        )
        f.write(
            str(float(cam_pose[2, 0])) + " " + str(float(cam_pose[2, 1])) + " "
            + str(float(cam_pose[2, 2])) + " " + str(float(cam_pose[2, 3])) + "\n"
        )
        f.write(
            str(float(cam_pose[3, 0])) + " " + str(float(cam_pose[3, 1])) + " " 
            + str(float(cam_pose[3, 2])) + " " + str(float(cam_pose[3, 3])) + "\n"
        )
    # fmt: on
