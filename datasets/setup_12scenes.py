"""Setup 12Scenes dataset.

See: https://www.csail.mit.edu/projects/graphics/scene-understanding/
"""
import argparse
import os
import zipfile
...
src_folder = "12scenes_raw"

target_folder = "12scenes"
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Download and setup the 12Scenes dataset.", formatter_class=argparse.ArgumentDefaultsHelpFormatter
    )
    ...
    parser.add_argument(
        ...
        default="pgt",
        help="original: original pose files; " "pgt: get SfM poses from external repository (Brachmann et al., ICCV21)",
    )
    ...
    def process_dataset(ds):
        ...
        for scene in scenes:
            ...
            if opt.poses == "pgt":
                target_folder = "../12scenes/pgt_" + ds + "_" + scene + "/"
            else:
                target_folder = "../12scenes/orig_" + ds + "_" + scene + "/"

            # create subfolders for training and test
            os.makedirs(target_folder + "test/rgb/", exist_ok=True)
            os.makedirs(target_folder + "test/poses/", exist_ok=True)
            os.makedirs(target_folder + "test/calibration/", exist_ok=True)
            if opt.depth == "original":
                os.makedirs(target_folder + "test/depth/", exist_ok=True)
            if opt.eye == "original":
                os.makedirs(target_folder + "test/eye/", exist_ok=True)

            os.makedirs(target_folder + "train/rgb/", exist_ok=True)
            os.makedirs(target_folder + "train/poses/", exist_ok=True)
            os.makedirs(target_folder + "train/calibration/", exist_ok=True)
            if opt.depth == "original":
                os.makedirs(target_folder + "train/depth/", exist_ok=True)
            if opt.eye == "original":
                os.makedirs(target_folder + "train/eye/", exist_ok=True)
            ...
            def link_frame(i, variant, pgt_poses):
                ...
                if not valid:
                    ...
                    # link image
                    os.system(f"ln -s ../../../../{src_folder}/{data_folder}/{images[i]} {target_folder}{variant}/rgb/")
                    ...
                    if opt.poses == "pgt":
                        ...
                        # link pose
                        os.system(
                            f"ln -s ../../../../{src_folder}/{data_folder}/{poses[i]} {target_folder}{variant}/poses/"
                        )

                    # create a calibration file
                    with open(
                        target_folder + variant + "/calibration/frame-%s.calibration.txt" % str(i).zfill(6), "w"
                    ) as f:
                    ...
                    if opt.depth == "original":
                        ...
                        os.system(
                            f"ln -s ../../../../{src_folder}/{data_folder}/{depth_maps[i]} {target_folder}{variant}/depth/"
                        )
    ...
    Parallel(n_jobs=4, verbose=0)(map(delayed(process_dataset), ["apt1", "apt2", "office1", "office2"]))
