"""Setup 7Scenes dataset.

See: https://www.microsoft.com/en-us/research/project/rgb-d-dataset-7-scenes/
"""
import argparse
import os
import warnings
...
if __name__ == "__main__":
    ...
    # fmt: off
    parser.add_argument("--setup_ace_structure", action="store_true", help="Create a copy of the dataset in the ACE format in 7scenes_ace. "
    ...
    parser.add_argument( "--poses", type=str, choices=["original", "calibrated", "pgt"], default="calibrated",
                        help="For ACE dataset format only. "
                             ...
                             "pgt (pseudo-groundtruth): get SfM poses from external repository (Brachmann et al., ICCV21)")
    # fmt: on
    ...
    def process_scene(ds):
        ...
        if opt.poses == "pgt":
            target_folder = f"../{ace_folder}/pgt_{ds}/"  # pgt: pseudo ground truth
        else:
            target_folder = f"../{ace_folder}/kf_{ds}/"  # kf: KinectFusion

        def link_frames(split_file, variant):

            # create subfolders
            os.makedirs(target_folder + variant + "/rgb/", exist_ok=True)
            os.makedirs(target_folder + variant + "/poses/", exist_ok=True)
            os.makedirs(target_folder + variant + "/calibration/", exist_ok=True)
            if opt.depth == "calibrated":
                os.makedirs(target_folder + variant + "/depth/", exist_ok=True)
            if opt.eye == "calibrated":
                os.makedirs(target_folder + variant + "/eye/", exist_ok=True)
            ...
            for seq in split:
                ...
                for img in images:
                    os.system(f"ln -s ../../../../{raw_folder}/{ds}/{seq}/{img} {target_folder}{variant}/rgb/{seq}-{img}")
                ...
                if opt.poses == "original":
                    ...
                    for p_file in pose_files:
                        os.system(
                            f"ln -s ../../../../{raw_folder}/{ds}/{seq}/{p_file} {target_folder}{variant}/poses/{seq}-{p_file}"
                        )
                ...
                if opt.poses == "pgt":
                    ...
                    for p_file in pose_files:
                        ...
                        dutil.write_focal_length(
                            target_folder + variant + "/calibration/" + seq + "-" + calib_file,
                            rgb_f,
                        )
                else:
                    for i in range(len(images)):
                        dutil.write_focal_length(
                            target_folder
                            + variant
                            + "/calibration/%s-frame-%s.calibration.txt" % (seq, str(i).zfill(6)),
                            focal_length,
                        )
                ...
                for d_file in depth_files:
                    ...
                    if opt.depth == "calibrated":
                        ...
                        with warnings.catch_warnings():
                            ...
                            io.imsave(
                                target_folder + variant + "/depth/" + seq + "-" + d_file,
                                registered_depth,
                            )
