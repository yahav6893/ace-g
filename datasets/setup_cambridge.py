...
def setup_scene(scene):
    ...
    src_scene_name = scene_file[:-4]
    target_scene_name = src_scene_name.lower()
    scene_dir = f"{out_dir}/{target_scene_name}"


    print("===== Processing " + target_scene_name + " ===================")
    ...
    os.system(f"mv {src_scene_name}/* {scene_dir}")
    os.system(f"rmdir {src_scene_name}")
    os.chdir(scene_dir)
    ...
    for mode in modes:
        ...
        for cam_idx in range(num_cams):
            ...
            np.savetxt(pose_output_folder + image_file[:-3] + "txt", inv_cam_pose.numpy())
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Download and setup the Cambridge dataset.", formatter_class=argparse.ArgumentDefaultsHelpFormatter
    )
