...
# -----------------------------
REGISTER_EVAL_TIMEOUT=10m  # kill bad evals automatically
FAIL_ON_EVAL_NAN=1         # fail run if eval emits NaN DSAC scores
...
# Stage controls.
# Use RUN_EVAL=0 to finish all trainings without entering registration/eval.
# Use RUN_TRAIN=0 to retry eval from existing status.json files only.
RUN_TRAIN="${RUN_TRAIN:-1}"
RUN_EVAL="${RUN_EVAL:-1}"

# Registration/eval guardrails.
# ACE-G registration should not take ~10 minutes per image. A timeout prevents one
# bad eval from blocking the whole sweep forever. Set REGISTER_EVAL_TIMEOUT=0 to disable.
REGISTER_EVAL_TIMEOUT="${REGISTER_EVAL_TIMEOUT:-45m}"

# If eval produces NaN DSAC/inlier scores, mark the run failed and continue.
FAIL_ON_EVAL_NAN="${FAIL_ON_EVAL_NAN:-1}"
...
run_pipeline_once() {
  ...
  if [[ "${RUN_TRAIN}" != "1" ]]; then
    log "RUN_TRAIN=${RUN_TRAIN}; skipping training stage and requiring existing status: ${status_json_path}"
  elif [[ "${RESUME}" == "1" && -f "${status_json_path}" ]]; then
  ...
  date '+%Y-%m-%d %H:%M:%S' > "${run_output_root}/train.done"

  if [[ "${RUN_EVAL}" != "1" ]]; then
    log "RUN_EVAL=${RUN_EVAL}; skipping registration/evaluation for ${run_id}."
    log "Training/status is available at: ${status_json_path}"
    return 0
  fi

  log ">>> STAGE 2: REGISTRATION & EVALUATION"
  local eval_log="${log_dir}/${run_id}.register_eval.log"

  set +e
  if [[ -n "${REGISTER_EVAL_TIMEOUT}" && "${REGISTER_EVAL_TIMEOUT}" != "0" ]]; then
    log "Applying register/eval timeout: ${REGISTER_EVAL_TIMEOUT}"
    timeout "${REGISTER_EVAL_TIMEOUT}" python "${REPO_ROOT}/scripts/register_eval.py" \
      --repo-root "${REPO_ROOT}" \
      --dataset-root "${DATASET_ROOT}" \
      --output-root "${run_output_root}" \
      --cache-root "${CACHE_ROOT}" \
      ...
      --wandb-entity "${WANDB_ENTITY}" \
      --wandb-project "${WANDB_PROJECT}" \
      --pythonpath-prepend "${PYTHONPATH_PREPEND}" 2>&1 | tee "${eval_log}"
  ...
  if [[ "${eval_status}" -ne 0 ]]; then
    echo "registration/evaluation failed or timed out with status ${eval_status}" > "${fail_marker}"
    return 1
  fi

  if [[ "${FAIL_ON_EVAL_NAN}" == "1" ]] && grep -Eiq 'Soft inlier count: -?nan|Selection Probability: -2147483648|\bnan\b' "${eval_log}"; then
    echo "registration/evaluation produced NaN/invalid DSAC scores" > "${fail_marker}"
    return 1
  fi
